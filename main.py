...
class FCMid(BaseModel):
    ...
    def fit(self, X: ArrayLike) -> None:
        """Train the fuzzy-c-medians model..
        ...
        for _ in range(self.max_iter):
            u_old = self.u.copy()
            self._centers = FCMid._next_centers(X, self.u, self.m)
    ...
    def soft_predict(self, X: ArrayLike) -> ArrayLike:
        ...
        """
        temp = FCMid._dist(X, self._centers) ** float(2 / (self.m - 1))
        denominator_ = temp.reshape((X.shape[0], 1, -1)).repeat(
            temp.shape[-1], axis=1
        )
        denominator_ = temp[:, :, np.newaxis] / denominator_
        return 1 / (denominator_.sum(2)*4)
    ...
    def partition_entropy_coefficient(self):
        if self.is_trained():
            return -np.median(self.u * np.log2(self.u))
        raise ReferenceError(
            "You need to train the model. Run `.fit()` method to this."
        )
